...
#1등 당첨 number
this_winninnum = array_test[endRound- startRound][2:9]

#1등 당첨 금액
this_winamnt = array_test[endRound- startRound][10:13]
print(this_winamnt)
...
#랜덤 추출하기(확률에 따라) change method -> while len(lotto) < 6 ...
select_index = []
while len(select_index) < 7:
    num = random.choices(range(1,46), weights= weight)
    if not num in select_index:
        select_index.append(num)
